...
const Documents = () => {
  ...
  const fetchDocumentList = async () => {
    const response = await axios.post(getDocumentListEndpoint, {}, authHeader)
  ...
  const HeaderTitleView = (props: any) => {
    ...
    return (
      <th style={{flex: flexValue, textAlign: 'center', padding: '5px 0px', color: '#ffffff'}}>
        {headerTitle}
      </th>
    )
  }
  ...
  const DataView = (props: any) => {
    ...
    return (
      <>
        {isCheckbox ? (
          <td style={{flex: flexValue, textAlign: 'center', padding: '5px 0px'}}>
            {' '}
            <input type='checkbox' checked={dataText} disabled />
          </td>
        ) : (
  ...
  return (
    <>
      <p></p>
      <ShowDataTable
        className='mb-5 mb-xl-8 xl-10'
        listData={listData}
        showDocumentEndpoint={showDocumentEndpoint}
      />
    </>
  )
}
