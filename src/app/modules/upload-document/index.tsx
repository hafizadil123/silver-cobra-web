...
const UploadDocumentSection = () => {
  ...
  const checkAvailableDates = async () => {
    const response = await axios.post(
      getDocumentsDetailsSaveEndpoint,
      {
  ...
  const postImageToServer = async () => {
    if (isValidated()) {
      ...
      selectedDocumentTypeId != 0 &&
        data.append('DocumentTypeCode', selectedDocumentTypeId.toString())
      description && data.append('description', description.toString())
      startDate &&
        data.append('AttendanceStartDate', moment(startDate).format('DD/MM/YYYY').toString())
      endDate && data.append('AttendanceEndDate', moment(endDate).format('DD/MM/YYYY').toString())
      file && data.append('file', file)
      marriageDate &&
        data.append('MarriageDate', moment(marriageDate).format('DD/MM/YYYY').toString())
      ...
      if (response && response.data) {
        ...
        if (!result) setErrorMessage(message)
    ...
    if (response && response.data.result) {
      ...
      setDocumentTypes(
        (documentCategoryTypes &&
          documentCategoryTypes.length &&
          getTheUpdatedCategoryTypes(documentCategoryTypes[0].documentTypes)) ||
          []
      )
  ...
  return (
    ...
    >
      <div className='upload_container'>
        <div className='inputView'>
          <div className='sectionOne'>
            <select
              ...
              onChange={(event: any) => {
                const selectedItem =
                  categoryTypes.filter((item: any) => item.id == event.target.value)[0] || {}
                setDocumentTypes(
                  getTheUpdatedCategoryTypes(selectedItem && selectedItem.documentTypes)
                )
          ...
          <div className='inputView'>
            <div className='sectionOne'>
              <select
                ...
                onChange={(event: any) => {
                  const selectedItem =
                    documentTypes.filter((item: any) => item.id == event.target.value)[0] || {}
        ...
        <div className='inputView'>
          <div className='sectionOne'>
            <input
              ...
              accept='.docx, .pdf, .png, .jpg,.jepg ,.gif'
            />
          </div>
          <p className='labelStyle'>{inputLabels.file}</p>
        </div>
        {selectedDocumentTypeId === 24 && (
          <div className='inputView'>
            <div className='sectionOne'>
              <input
                type='date'
          ...
          <div className='inputView'>
            <div className='sectionOne'>
              <select
                ...
                onChange={(event: any) => {
                  const selectedItem =
                    hmoTypes.filter((item: any) => item.id == event.target.value)[0] || {}
          ...
          <div className='inputView'>
            <div className='sectionOne'>
              <select
                ...
                onChange={(event: any) => {
                  const selectedItem =
                    banks.filter((item: any) => item.id == event.target.value)[0] || {}
        ...
        {[23, 40].includes(selectedDocumentTypeId) && (
          <div className='inputView'>
            <div className='sectionOne'>
              <input
                type='month'
      ...
      {errorMessage && (
        <div
          ...
          style={{background: responseStatus ? '#4CAF50' : '#EF5350'}}
        >
          {errorMessage}
        </div>
      )}
